...
// List of commands to be processed by the console.
const commands =
{
    ...
    "format drv mffs" : "Format what drive?",

    "format drv" : "Please specify filesystem.",
    ...
    "access drv" : "Access what drive?",

    // Default output for unknown commands
    "default" : "Command not registered. Type 'help' or '?' for a list of basic commands. File names are not case-sensitive."
};

// Take in the command the user enters (userInput) and output the relvant text from commands
function processCommand(userInput)
{
    output.innerHTML += `Remote$User> ${userInput}<br>`; // Show the command in the log
    ...
    if (commands[userInput.toLowerCase()])
    {
      ...
      {
        output.innerHTML += `${commands[userInput.toLowerCase()].replace(/\n/g, '<br>')}<br>`;
      }
    }
    else
    {
